...
(defn- singles-section [{:keys [set-attr remove-attr ext]} singles]
  [:div
   (for [{:keys [id name collection mapped-to]} singles]
     [attribute name {:on-change #(set-attr id (-> % .-target .-value))
                      :on-remove #(remove-attr id)}
      [single-attr {:collection               collection
                    :mapped-to           mapped-to
                    :on-collection-change     #(rf/dispatch [:mapping/>set-ext-single-collection (:id ext) id %])
   ...
   (for [{:keys [id name sub-attrs]} objects]
     ...
     [attribute name {:on-change #(set-attr id (-> % .-target .-value))
      ...
      [object-attr
       ...
       (for [{sub-attr-id :id sub-name :name mapped-to :mapped-to collection :collection} sub-attrs]
         ^{:key sub-attr-id}
         [sub-attr sub-name
          ...
          [object-inputs
           {:collection               collection
            ...
            :on-collection-change     #(rf/dispatch [:mapping/>set-ext-object-collection (:id ext) id sub-attr-id (-> % .-target .-value)])}]])]])])
       ...
       (for [{sub-item-id :id mapped-to :mapped-to type :type collection :collection} sub-items]
         ...
         [array-attr-item
          {:collection               collection
           ...
           :on-collection-change     #(rf/dispatch [:mapping/>set-ext-array-collection (:id ext) id sub-item-id (-> % .-target .-value)])
